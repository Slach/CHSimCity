...
[![CHSimCity: one server's parts yard, with system.parts towers, the mark cache
and a running merge](docs/screenshot.png)](https://slach.github.io/CHSimCity/)

One server's parts yard in the day theme: each tower is a row in `system.parts`,
coloured by its state; the orange slab behind it is the mark cache, and
`system.merges` in the foreground had one merge running when the shot was taken.
