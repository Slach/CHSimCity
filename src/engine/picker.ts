 ...
 * `focus` on the bus: raycast at 25 Hz against the registry roots, never while
 ...
 * set of corner brackets looks like a gun sight, so the marker is neither: it is
 * the drawing an engineer would hand you for this one object. A setting-out
 ...
 * dimension line on each of the two exposed sides. Under all of it, a soft band
 * of light on the plot, which is what makes the selection read from 600 m away.
 *
 * Nothing about the marker moves. The only animation anywhere is a slow rise and
 * fall in the ground light's brightness, at the rate a lamp warms.
 *
 ...
 * instanced-mesh instances far below the world so they cannot be seen, and a
 * marker sized from those is a kilometre tall. So an impossible box is not
 * accepted: it is split into the object's children and those are measured
 * instead, and the boxes that survive are unioned back together.
 * -------------------------------------------------------------------------*/

/** Taller than this and it is a parking artefact. An island plus its pit is 130. */
const MAX_H = 200
/** Wider than this in X or Z and it is not one object either. */
const MAX_SPAN = 3200
/** Boxes entirely outside this band are off the map. A pit floor is -45. */
const Y_FLOOR = -300
const Y_CEIL = 320
...
/** Keep the top of the box and a sane footprint around it. */
...
function measure(obj: THREE.Object3D, out: THREE.Box3): boolean {
  ...
  // Every part was rejected. Use the raw box, clipped to something drawable,
  // rather than leaving the user with no marker at all.
  _sub.makeEmpty()
  _sub.setFromObject(obj)
...
export interface PickerApi {
  /** Selection and hover markers live here. */
...
export function createPicker(opts: {
  ...
  function onPointerUp(ev: PointerEvent): void {
    ...
    if (id !== null && id === lastClickId && nowMs - lastClickT < DBL_MS) {
      bus.emit('focus', { id })
      lastClickT = -1e9
      lastClickId = null
    } else {
  ...
  /**
   * Fit a marker to a component. Not cheap (it traverses the subtree, twice over
   * for a container that has to be split), which is exactly why it runs on
   * selection change and twice a second, not per frame.
   */
  function applyBox(m: Marker, def: ComponentDef): void {
    ...
    const pad = clamp(maxDim * 0.06, 0.6, 5)
    const tick = clamp(maxDim * 0.1, 0.5, 7)
    ...
    // gets it just beneath its own base — a soffit plan — so the marker is never
    // a 40 m line hanging off the object.
    const base = _box.min.y
    const planY = base > -1 && base < 14 ? 0.08 : base - Math.max(1.5, pad)
...
function makeMarker(
  ...
  const mat = new THREE.LineBasicMaterial({
    ...
    transparent: true,
    opacity,
    // A marker you cannot see through the object it marks is a bug, not a
...
function noRaycast(): void {
  /* markers are chrome — never pickable */
}

/**
 * Unit-radius setting-out circle in the XZ plane: one continuous ring plus
 * outward radial ticks at the eighths, which is how a plot is marked out before
 * anything is built on it. Static — it never turns.
 */
 ...
 * shader. Additive, so on a dark plate it reads as a lit kerb rather than a disc
 * lying on the ground.
 */
