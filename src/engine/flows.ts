...
/* ============================================================================
 * FLOWS — every moving packet in the cluster.
 *
 * An INSERT arriving at the initiator, a sorted block heading for the column
 ...
 *     an arc-length LUT and allocates internally — calling it thousands of times
 *     per frame is not survivable, so we sample 96 points up front and lerp.
 * ==========================================================================*/

/** Polyline resolution per route. 96 segments is smooth at cluster scale. */
const SAMPLES = 96
...
/** Index 13 is the "no kind given" default. */
const KIND_DEFAULT = KIND_ORDER.length
// Width and length per kind. A `fetch` is the biggest thing that moves in this
// model, because it really is: a whole part directory over HTTP.
const KIND_W = Float32Array.from([1.05, 1.25, 1.35, 1.0, 0.95, 0.8, 1.15, 1.4, 1.2, 1.5, 0.7, 1.3, 0.7])
...
/**
 * The freight pod: a chamfered four-sided body, very nearly as fat as it is
 * long. Eighteen vertices, thirty-two triangles.
 *
 * This shape is load-bearing. A tapered dart oriented along travel and stretched
...
function packetGeometry(): THREE.BufferGeometry {
  ...
  const pos: number[] = []
  const col: number[] = []
  /** One four-vertex ring on the +X/+Y/-X/-Y axes. */
...
export function createFlows(
  ...
  function buildPool(size: number): void {
    ...
    // Instances span the entire cluster; a geometry-derived bounding sphere
    // would cull the whole batch the moment the camera looks away from origin.
    ...
    // update(). An idle cluster costs nothing.
    mesh.count = 0
    group.add(mesh)
  ...
  function spawn(route: number, color: number, speed: number, size: number, spread: number, kind: number): void {
    if (nFree === 0) {
      dropped++ // pool exhausted — the cluster is busier than the budget allows
      return
    }
    const i = free[--nFree]
    act[nAct++] = i
  ...
  function emit(req: FlowRequest): void {
    ...
    // The first packet leaves now, the rest are spread across `stagger` seconds.
  ...
  function update(dt: number): void {
     ...
     * low end of the pool, so on a quiet cluster this is a couple of hundred out
     * of a couple of thousand, and the rest is never submitted. */
    let hi = 0
  ...
  function setQuality(q: QualitySettings): void {
    ...
    // the cluster. Keep enough to read the routes, drop the rest.
