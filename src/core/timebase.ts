/** The renderer and the animated world never consume more than this per frame. */
export const MAX_VISUAL_DELTA_SECONDS = 0.1
/**
 * A two-second frame is possible on a software renderer. Larger gaps are
...
export function simulationAnimationDelta(animationDt: number, paused: boolean, timeScale: number): number {
  return paused ? 0 : animationDt * timeScale
}

/**
 * Keep model time synchronised to accepted wall time without ever giving the
 * model one enormous update. Pausing drops the fractional backlog immediately,
 * so unpausing does not replay the gap.
 */
