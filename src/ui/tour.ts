...
export const CHAPTERS: TourChapter[] = [
  {
    ...
    body: 'Two shards, two replicas each, three Keeper nodes, and one Distributed table in front of them. The geography is the order things happen in: writes arrive from the north, the data lives in the middle, and the metadata that coordinates it all is in the south.',
    focus: 'cluster',
    duration: 15,
  ...
  {
    id: 'clients',
    title: 'The client knows about one table',
    body: 'The application tier stands outside the cluster and talks to exactly one thing. It has no idea how many shards there are, and adding one changes nothing here. The pallet stack on the west dock is the INSERT batch size — the most important number a ClickHouse client controls.',
    focus: 'clients',
    duration: 15,
  },
  {
    id: 'distributed',
    title: 'The Distributed table stores nothing',
    body: 'It is a router with a query rewriter. The wheel is the sharding expression: a block arrives, the wheel turns, and the block leaves in two pieces. The arc widths are the cumulative share each shard has received, so a badly chosen sharding key becomes visible rather than merely suspected.',
